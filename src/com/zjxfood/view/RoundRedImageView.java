 ...
 * 
 */
public class RoundRedImageView extends ImageView {
	...
	public RoundRedImageView(Context paramContext) {
		super(paramContext);
	}
	
	public RoundRedImageView(Context paramContext, AttributeSet paramAttributeSet) {
		this(paramContext, paramAttributeSet, 0);
	}

	public RoundRedImageView(Context context, Bitmap mask, Paint paint,
		int mBorderWidth, int mBorderColor, boolean useDefaultStyle) {
	super(context);
	this.mask = mask;
	this.paint = paint;
	...
	public RoundRedImageView(Context paramContext, AttributeSet paramAttributeSet,
		...
		mBorderColor = a.getColor(R.styleable.CircularImage_border_color,
				mBorderColor);
		final int defalut = (int) (2 * paramContext.getResources()
				.getDisplayMetrics().density + 0.5f);
		mBorderWidth = a.getDimensionPixelOffset(
				R.styleable.CircularImage_border_width, defalut);
		a.recycle();
	}
	...
	private void drawBorder(Canvas canvas, final int width, final int height) {
		...
		mBorderPaint.setColor(getResources().getColor(R.color.red_color4));
	...
	public Bitmap createOvalBitmap(final int width, final int height) {
		...
		final int padding = mBorderWidth;
		/**
