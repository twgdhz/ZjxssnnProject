...
public class MyIndianaListAdapter extends BaseAdapter{
	...
	public MyIndianaListAdapter(Context context,ArrayList<HashMap<String, Object>> list){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(mList.get(position).get("ProductName")!=null && mList.get(position).get("Id")!=null){
			mHolder.mNameText.setText("【第"+mList.get(position).get("Id")+"期】"+mList.get(position).get("ProductName").toString());
		}
...
//		}
		if(mList.get(position).get("LuckyNum")!=null){
			if(mList.get(position).get("LuckyNum").toString().equals("0")){
				mHolder.mButton.setBackgroundResource(R.drawable.shape_indiana_cj_bg);
				mHolder.mButton.setTextColor(mContext.getResources().getColor(R.color.indiana_item_choujiang_coloe2));
				mHolder.mButton.setText("进行中");
			}else{
