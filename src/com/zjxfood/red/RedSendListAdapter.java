...
public class RedSendListAdapter extends BaseAdapter{
	private LayoutInflater mInflater;
	private Context mContext;
	private ArrayList<HashMap<String, Object>> mList;
	
	public RedSendListAdapter(Context context,ArrayList<HashMap<String, Object>> list){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			mHolder.mNumbersText = (TextView) convertView.findViewById(R.id.red_issued_bottom_text2);
		...
		if(mList.get(position).get("TotalMoney")!=null){
			mHolder.mPriceText.setText(mList.get(position).get("TotalMoney").toString()+"元");
		}
		if(mList.get(position).get("SendOn")!=null){
			mHolder.mDateText.setText(mList.get(position).get("SendOn").toString());
		}
		if(mList.get(position).get("TotalNum")!=null && mList.get(position).get("RemainNum")!=null){
			...
			mHolder.mNumbersText.setText(a+"/"+mList.get(position).get("TotalNum")+"个");
		}
		return convertView;
	}
	class ViewHolder{
